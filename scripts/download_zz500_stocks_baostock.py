"""
使用 BaoStock 下载中证500成分股（zz500Stock）。

接口文档：
http://www.baostock.com/mainContent?file=zz500Stock.md
...
示例：
python scripts/download_zz500_stocks_baostock.py
python scripts/download_zz500_stocks_baostock.py --date 2026-02-27
python scripts/download_zz500_stocks_baostock.py --start-date 2026-02-01 --end-date 2026-02-28
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="下载中证500成分股（BaoStock）")
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="ai_stock_agent/data/zz500",
        help="输出目录",
    )
    return parser.parse_args()


def fetch_zz500_for_date(date: str | None) -> pd.DataFrame:
    rs = bs.query_zz500_stocks(date=date or "")
    if rs.error_code != "0":
        raise RuntimeError(f"query_zz500_stocks failed: {rs.error_code} {rs.error_msg}")
    ...
    try:
        if args.start_date or args.end_date:
            ...
            for d in trade_dates:
                day_df = fetch_zz500_for_date(d)
                if not day_df.empty:
                    all_df.append(day_df)

            if not all_df:
                raise RuntimeError(f"未获取到中证500成分股数据: {args.start_date}~{args.end_date}")

            df = pd.concat(all_df, ignore_index=True)
            df = df.drop_duplicates(subset=["query_date", "code"]).reset_index(drop=True)
            out_file = out_dir / f"zz500_stocks_{args.start_date}_{args.end_date}.csv"
        else:
            df = fetch_zz500_for_date(args.date)
            if df.empty:
                target_date = args.date or "latest"
                raise RuntimeError(f"未获取到中证500成分股数据: {target_date}")

            suffix = (args.date or "latest").replace("-", "")
            out_file = out_dir / f"zz500_stocks_{suffix}.csv"
    finally:
        bs.logout()
