"""
分步下载全市场历史 daily 快照（按交易日）

示例：
python scripts/download_daily_history.py --start-date 20260201 --end-date 20260228
"""
...
from config.settings import validate_config, DAILY_HISTORY_DIR  # noqa: E402
from tushare_api.downloader import TushareDownloader  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="按交易日下载全市场 daily 历史数据")
    ...
    parser.add_argument(
        "--output-dir",
        default=str(DAILY_HISTORY_DIR),
        help="输出目录（默认 ai_stock_agent/data/daily_history）",
    )
    parser.add_argument(
        "--sleep-seconds",
        type=float,
...
def main() -> None:
    ...
    for trade_date in trade_dates:
        ...
        try:
            df = downloader.get_daily_snapshot(trade_date)
            if df is None:
                failed.append(trade_date)
                continue

            downloader.save_daily_snapshot_csv(df, trade_date, output_dir=output_dir)
