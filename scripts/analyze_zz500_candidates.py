"""
分析 ZZ500 最新成分股，剔除银行/保险/证券，并筛选可加入 stock_pool 的候选。

输入：
- ai_stock_agent/data/zz500/zz500_stocks_latest.csv
...
输出：
- ai_stock_agent/data/zz500/zz500_no_financial.csv
- ai_stock_agent/data/zz500/zz500_candidates_for_pool.csv
- ai_stock_agent/data/zz500/zz500_candidates_focus.csv
- ai_stock_agent/data/zz500/zz500_candidates_summary.json
"""
...
def main() -> None:
    zz500_path = DATA_DIR / "zz500" / "zz500_stocks_latest.csv"
    stock_info_path = DATA_DIR / "all_stocks_active_no_st.csv"
    stock_pool_path = DATA_DIR / "stock_pool.json"

    out_dir = DATA_DIR / "zz500"
    out_dir.mkdir(parents=True, exist_ok=True)

    zz = pd.read_csv(zz500_path)
    base = pd.read_csv(stock_info_path)

    zz["ts_code"] = zz["code"].map(normalize_ts_code)
    ...
    no_financial_out = out_dir / "zz500_no_financial.csv"
    candidates_out = out_dir / "zz500_candidates_for_pool.csv"
    focus_out = out_dir / "zz500_candidates_focus.csv"
    summary_out = out_dir / "zz500_candidates_summary.json"
